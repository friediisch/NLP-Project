...
wv = data[:,:300] # 11 features
tfidf = data[:, 307:308]
data = np.hstack((wv, tfidf, docvec))
...
# define the model
model = Sequential()
model.add(Dense(data.shape[1], input_dim=data.shape[1], kernel_regularizer=regularizers.l2(0.01)))
model.add(Dropout(0.2, input_shape=(data.shape[1],)))
model.add(Dense(500, activation='relu', kernel_regularizer=regularizers.l2(0.01)))
model.add(Dropout(0.1, input_shape=(500,)))
model.add(Dense(300, activation='relu', kernel_regularizer=regularizers.l2(0.01)))
model.add(Dropout(0.1, input_shape=(300,)))
...
df.to_csv('../data/models/keras/results_model6.csv', sep=';')
...
model.save('../data/models/keras/model6.h5')
